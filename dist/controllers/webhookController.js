...
class WebhookController {
    async processWebhook(req, res) {
        ...
        try {
            ...
            if (!this.verifyWebhookSignature(providerString, payload, signature)) {
                ...
                throw new types_1.WebhookSignatureError('Invalid webhook signature');
            }
    ...
    async getWebhookEvents(req, res) {
        ...
        const { provider } = req.params;
        const providerString = Array.isArray(provider) ? provider[0] : provider;
        const { processed, eventType, limit, offset, startDate, endDate } = req.query;
        try {
            const where = { provider: providerString };
    ...
    async processWebhookEvent(provider, payload, eventId) {
        try {
    ...
    async processPaystackWebhook(payload) {
        ...
        switch (event) {
            ...
            case 'transfer.success':
                ...
                if (data.reference) {
                    await transactionService_1.transactionService.processTransaction(data.reference, {
            ...
            case 'transfer.failed':
                ...
                if (data.reference) {
                    await transactionService_1.transactionService.failTransaction(data.reference, data.message || 'Payment failed');
                }
                break;
            default:
    ...
    async processFlutterwaveWebhook(payload) {
        ...
        switch (event) {
            case 'charge.completed':
                ...
                if (data.txRef) {
                    await transactionService_1.transactionService.processTransaction(data.txRef, {
            ...
            case 'charge.failed':
                ...
                if (data.txRef) {
                    await transactionService_1.transactionService.failTransaction(data.txRef, data.status || 'Payment failed');
                }
                break;
            default:
        ...
        switch (type) {
            case 'payment_intent.succeeded':
                ...
                if (data.metadata?.reference) {
                    await transactionService_1.transactionService.processTransaction(data.metadata.reference, {
                        providerRef: data.id,
                        metadata: {
                            provider: 'stripe',
                            event: type,
            ...
            case 'payment_intent.payment_failed':
                // Process failed payment
                if (data.metadata?.reference) {
                    await transactionService_1.transactionService.failTransaction(data.metadata.reference, data.last_payment_error?.message || 'Payment failed');
                }
                break;
            default:
