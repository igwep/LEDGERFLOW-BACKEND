 ...
 */
export async function generatePaymentEndpoint(username?: string): Promise<string> {
  let endpoint: string;
  let attempts = 0;
 ...
 */
function createEndpointString(username?: string): string {
 ...
 */
async function isEndpointTaken(endpoint: string): Promise<boolean> {
  try {
    const existingUser = await prisma.user.findUnique({
      where: { 
        paymentEndpoint: endpoint as any // Temporary fix for type issue
      },
      select: { id: true }
    });
 ...
 */
export function validateEndpointFormat(endpoint: string): boolean {
 ...
 */
export async function generateEndpointOptions(username?: string, count: number = 3): Promise<string[]> {
 ...
 */
export async function validateEndpointOwnership(endpoint: string): Promise<{
  isValid: boolean;
  userId?: string;
  userStatus?: string;
}> {
