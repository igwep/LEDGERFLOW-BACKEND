...
export class WalletService {
  async createWallet(userId: string, currency: string = 'NGN'): Promise<ServiceResponse<WalletData>> {
    try {
  ...
  async getWallet(userId: string): Promise<ServiceResponse<WalletData>> {
    try {
  ...
  async processDoubleEntryTransaction(
    walletId: string,
    ...
    description?: string,
    reference?: string
  ): Promise<ServiceResponse<LedgerEntryData[]>> {
    return await prisma.$transaction(async (tx: PrismaClientType) => {
      try {
        ...
        if (wallet.status !== 'ACTIVE') {
          throw new WalletLockedError('Wallet is not active');
        }
  ...
  async lockFunds(
    ...
    return await prisma.$transaction(async (tx: PrismaClientType) => {
      try {
        ...
        if (currentAvailable < amount) {
          throw new InsufficientFundsError('Insufficient available balance to lock funds');
        }
    ...
    return await prisma.$transaction(async (tx: PrismaClientType) => {
      try {
        ...
        if (currentLockedBalance < amount) {
          throw new InsufficientFundsError('Insufficient locked balance to unlock');
        }
  ...
  ): Promise<ServiceResponse<{ entries: LedgerEntryData[]; total: number }>> {
    try {
  ...
  private mapWalletToData(wallet: any): WalletData {
    return {
      id: wallet.id,
  ...
  private mapLedgerEntryToData(entry: any): LedgerEntryData {
    return {
      id: entry.id,
