...
export class WalletController {
  ...
  async getWallet(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const result = await walletService.getWallet(userIdString);
  ...
  async getLedgerEntries(req: AuthenticatedRequest, res: Response) {
    try {
      const { userId } = req.params;
      const userIdString = Array.isArray(userId) ? userId[0] : userId;
      const { 
      ...
      // Get wallet first
      const walletResult = await walletService.getWallet(userIdString);
      ...
      const result = await walletService.getLedgerEntries(walletResult.data!.id, {
        limit: limit ? parseInt(String(limit)) : undefined,
        offset: offset ? parseInt(String(offset)) : undefined,
        type: type ? String(type) as 'CREDIT' | 'DEBIT' : undefined,
  ...
  async lockFunds(req: AuthenticatedRequest, res: Response) {
    try {
      const { userId } = req.params;
      const userIdString = Array.isArray(userId) ? userId[0] : userId;
  ...
  async unlockFunds(req: AuthenticatedRequest, res: Response) {
    try {
      const { userId } = req.params;
      const userIdString = Array.isArray(userId) ? userId[0] : userId;
      const { amount, description } = req.body;
      ...
      const result = await walletService.unlockFunds(
        walletResult.data!.id,
        amount,
        description
      );
